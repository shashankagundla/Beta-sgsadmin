...
class ImageServer
{
    ...
    */
    public static function showThumbnail($file, $inline = false)
    {
        ...
        if (!is_dir($thumbsdir)) {
            ...
            if (false == (file_put_contents($thumbsdir.'/.htaccess', 'Options -Indexes'))
            ) {
                Utils::setError('error creating .htaccess for /vfm-admin/thumbs/');
            }
        }
    ...
    */
    public static function openImage($file)
    {
        ...
        $defaultimg = imagecreatefromjpeg('vfm-admin/images/placeholder.jpg');
...
class Updater
{
    ...
    */
    public function findUserKey($userdata)
    {
        ...
        foreach ($utenti as $value) {
            if ($value['key'] === $userdata) {
                $thisuser = array();
                $thisuser['name'] = $value['name'];
                $thisuser['pass'] = $value['pass'];
                ...
                if ($defaultfolders) {
                    ...
                    if (in_array('vfm_reg_new_folder', $arrayfolders)) {
                        ...
                        $userdir = json_encode(array($value['name']));
                    } else {
...
class GateKeeper
{
    ...
    */
    public static function getAvatar($username)
    {
        ...
        return SetUp::getConfig('script_url').$avaimg;
    }
    ...
    public static function renameFile($oldname, $newname, $thenewname, $move = false)
    {
        ...
        if (!file_exists($newname)) {
            if (file_exists($oldname) && !rename($oldname, $newname)) {
                Utils::setError('ERROR editing: <strong>' .$thenewname. '</strong>');
                return false;
            } else {
                if ($move === true) {
            ...
            if ($open_dir = opendir($fullpath)) {
                ...
                while ($object = readdir($open_dir)) {
                    if ($object != '.' && $object != '..') {
                        ...
                        } elseif (!in_array($object, SetUp::getConfig('hidden_files'))
                            && $object != 'vfm-admin'
                        ) {
    ...
    */
    public static function checkDel($path)
    {
        ...
        if (md5($del.SetUp::getConfig('salt').SetUp::getConfig('session_name')) === $cash) {
            ...
            $evil = array('', '/', '\\', '.');
            $avoid = array('.htaccess', 'index.php', 'vfm-thumb.php', 'vfm-admin', $basedir);
