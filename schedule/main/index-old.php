...
<script>
    function initMap() {
        ...
        marker = new google.maps.Marker({
            ...
            map: map,
            <?php if ($row['crew_1'] AND $row['crew_1'] != 'null' AND $row['crew_1'] != '26'){ ?>
            icon: 'http://www.googlemapsmarkers.com/v1/<?=substr($row['job_type'], 0, 1)?>/<?=substr($row['color'], 1, 7)?>'
            <?php
            }else{?>
            icon: 'http://www.googlemapsmarkers.com/v1/<?=substr($row['job_type'], 0, 1)?>/d3d3d3'
            <?php }?>
        });
